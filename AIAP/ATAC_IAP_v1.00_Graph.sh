...
# read parameters
while getopts m:t:g:o:p:r:i:c:s:u:h  opts
do case "$opts" in
...
#### Since I use absolute paths, I am had to add code that extracts the filename from the provide path for reads.
if [[ $R1 == *.sra ]]
    then name=`echo ${R1%.sra}`
    echo "this is sra file, $fastq_dump_tool would be used......"
    elif [[ $R1 == *.fastq* || *.fq.gz || *.fastq.gz ]] && [[ $types == PE  ]]
        then
        #name=`echo ${R1%.fastq*}`
	#name=`echo ${name%.fq.gz}_$species`
	name=`echo $sample"_"$replicate"_"$species`
        raw1=$R1
        raw2=$R2
	R1=`basename $R1`
	R2=`basename $R2`

    elif [[ $R1 == *.fastq* || *.fq.gz || *.fastq.gz ]] && [[ $types == SE ]]
        then
        #name=`echo ${R1%.fastq*}`
...
s2.0_ref () {
	...
	### The way it works is that it is removing entries with a name longer than 6 characters by default. I am going to change it to 60.
    awk  '{ if ((length($1) < 60) && (length($1) > 1))  print $0}' OFS='\t' $chrom_size  > refined_chrom_size.txt
...
s2.2_distri () {
    ...
    ## We need to make 'step2.1_trimed_'$name'.bam' match the reads from the graph alignment

    # This line should count up the number of uniquely mapped reads per chromosome. I am not yet sure how haplotype will play a role here. Given the surjection process.
    ...
    #I am not sure how chrM reads will behave given the graph
...
s4.6_visualization () {
    ...
    # I am disabling the deletion of the refined_chrom_size.txt file because the code deletes it on line 346 before this.
