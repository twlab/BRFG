---
title: "RNA-seq Samtools stats alignment quality analysis - split by chromosome"
author: "CIG @ MGI"
date: "`r Sys.Date()`"
...
knitr::opts_knit$set(root.dir = "/Users/juanmacias/Library/CloudStorage/Box-Box/LawsonLab/JuanMacias/Postdoc/HPRC/RNAseq/AlignmentQuality_Analysis/")
...
LinearGenome_file_list <- dir("/Users/juanmacias/Library/CloudStorage/Box-Box/LawsonLab/JuanMacias/Postdoc/HPRC/RNAseq/AlignmentQuality_Analysis/LinearAlignment_SamtoolsStats/AlignmentQualityMetricsSplit", full.names = TRUE)

# start to extract run information from file name
LinearGenome_Sample_Information<-unlist(lapply(LinearGenome_file_list, function(X) strsplit(X,"/")[[1]][15]))

# remove ".alignmentQualityMetrics" from LinearGenome_Sample_Information
LinearGenome_Sample_Information <- gsub(".alignmentQualityMetrics", "", LinearGenome_Sample_Information)
# remove "_v29" from LinearGenome_Sample_Information
LinearGenome_Sample_Information <- gsub("gencode_v29", "gencode", LinearGenome_Sample_Information)
# correct replicate delimiting 
LinearGenome_Sample_Information <- gsub("_1.", ".A.", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("_2.", ".B.", LinearGenome_Sample_Information)
# rename CHM13v2.0 to CHM13
LinearGenome_Sample_Information <- gsub("CHM13v2.0", "CHM13", LinearGenome_Sample_Information)
# rename NA annotations as none
LinearGenome_Sample_Information <- gsub("NA", "none", LinearGenome_Sample_Information)

# replaced any Assembly values containing .maternal with the name Maternal.
LinearGenome_Sample_Information <- gsub("HG00621_mat", "Maternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG00741_mat", "Maternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG01952_mat", "Maternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG01978_mat", "Maternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG03516_mat", "Maternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG00621_pat", "Paternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG00741_pat", "Paternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG01952_pat", "Paternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG01978_pat", "Paternal", LinearGenome_Sample_Information)
LinearGenome_Sample_Information <- gsub("HG03516_pat", "Paternal", LinearGenome_Sample_Information)

# Fix chromosome names
#LinearGenome_Sample_Information <- paste(unlist(lapply(LinearGenome_Sample_Information, function(X) strsplit(X, "\\.")[[1]][1])),gsub("_",".",unlist(lapply(LinearGenome_Sample_Information, function(X) strsplit(X, "\\.")[[1]][2]))),sep = "_")

# split the string by "_" to make columns: Sample, Replicate, Assembly, Annotations and turn into a data frame
LinearGenome_Sample_Information <- as.data.frame(do.call(rbind, strsplit(LinearGenome_Sample_Information, "\\.")))
colnames(LinearGenome_Sample_Information) <- c("Sample", "Replicate", "Assembly", "Annotations", "Chromosome")
...
lapply(
  ...
  function(X) read_stats(LinearGenome_file_list[X], sample=LinearGenome_Sample_Information[X,]$Sample, replicate=LinearGenome_Sample_Information[X,]$Replicate, assembly=LinearGenome_Sample_Information[X,]$Assembly, annotations=LinearGenome_Sample_Information[X,]$Annotations, chromosome=LinearGenome_Sample_Information[X,]$Chromosome)
...
LinearGenome_Stats<-subset(LinearGenome_Stats, Chromosome %in% c("chr1", "chr2", "chr3", "chr4", "chr5", "chr6", "chr7", "chr8", "chr9", "chr10", "chr11", "chr12", "chr13", "chr14", "chr15", "chr16", "chr17", "chr18", "chr19", "chr20", "chr21", "chr22", "chrX", "chrY", "chrM"))
...
# Read in list of all files in the directory with full path
Pangenome_file_list <- dir("/Users/juanmacias/Library/CloudStorage/Box-Box/LawsonLab/JuanMacias/Postdoc/HPRC/RNAseq/AlignmentQuality_Analysis/PangenomeAlignment_SamtoolsStats/AlignmentQualityMetricsSplit", full.names = TRUE)

# start to extract run information from file name
Pangenome_Sample_Information<-unlist(lapply(Pangenome_file_list, function(X) strsplit(X,"/")[[1]][15]))
...
# replace _1_ with _A_ and _2_ with _B_
Pangenome_Sample_Information <- gsub("_1_", "_A_", Pangenome_Sample_Information)
Pangenome_Sample_Information <- gsub("_2_", "_B_", Pangenome_Sample_Information)

# replace .gencode with _gencode
Pangenome_Sample_Information <- gsub(".gencode.GRCh38", "_gencode", Pangenome_Sample_Information)
...
colnames(Pangenome_Sample_Information) <- c("Sample", "Replicate", "Assembly", "Annotations", "Chromosome")

# replace "." with "_" in the chromosome column
Pangenome_Sample_Information$Chromosome <- gsub("\\.", "_", Pangenome_Sample_Information$Chromosome)
...
lapply(
  ...
  function(X) read_stats(Pangenome_file_list[X], sample=Pangenome_Sample_Information[X,]$Sample, replicate=Pangenome_Sample_Information[X,]$Replicate, assembly=Pangenome_Sample_Information[X,]$Assembly, annotations=Pangenome_Sample_Information[X,]$Annotations, chromosome=Pangenome_Sample_Information[X,]$Chromosome)
...
for (METRIC in unique(all_data.long$Metric)){
  ...
  try(
    print(summary(aov(Value~Assembly*Sample*Annotations, TEST)))
  )
}
```

```{r plot Metrics gencode, include=FALSE}
# Metrics to plot:
# mismatches
# error_rate
...
#all_data.wide.gencode<-subset(all_data.wide, Annotations=="gencode")
#all_data.wide.gencode<-all_data.wide
...
Plot_error_rate<-ggplot(all_data.wide, aes(x=Assembly, y=reads_mapped))+
  geom_boxplot(outliers = FALSE, color="darkgrey")+
  geom_jitter(width=0.2, aes(color=Sample, shape=Sample))+
  scale_color_viridis(discrete=TRUE, begin=0.1 ,end = 0.9, option = "plasma")+
...
CombinedPlot.gencode<-plot_grid(CombinedPlot, legendplot, ncol=1, rel_heights = c(1, 0.05))
```
