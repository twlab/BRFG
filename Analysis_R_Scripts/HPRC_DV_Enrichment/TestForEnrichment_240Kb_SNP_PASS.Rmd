---
title: "Testing 240Kb genomic regions for elevated rates of SNP PASS calls"
author: "CIG @ MGI"
date: "`r Sys.Date()`"
...
Real.Data<-read.delim("DV_SNPs_Intersection_Real_With_240kbBins_NoDups.bed", header = FALSE)
colnames(Real.Data)<-c("Chr","Start","Stop","Count")
```

```{r read in permuted data, include=FALSE}
Shuffled.Data<-read.delim(gzfile("Combined_DV_SNPs_Intersection_Shuffles_With_240kbBins.bed.gz"), header = FALSE)
...
ggsave(filename = "NullDistPlot_SNP_PASS_240kb.png",plot = NullDistPlot,units = "in", width = 12, height = 12)
...
ggsave(filename = "RealDataPlot_SNP_PASS_240kb.png",plot = RealDataPlot,units = "in", width = 12, height = 12)

RealDataPlot
```

```{r Save table of enrichment, include=FALSE}
write.table(x = Real.Data, file = "240kb_Regions_Enriched_in_SNP_PASS_AllData.txt", quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE)
write.table(x = subset(Real.Data, Significant==TRUE), file = "240kb_Regions_Enriched_in_SNP_PASS_SigOnly.txt", quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE)
```
...
ggsave(filename = "240kb_ManhattenLikePlots_Regions_Enriched_in_SNP_PASS.png", plot = ManhattenPanel, units = "in", width = 12, height = 12)
```
