---
title: "Testing 24kb genomic regions for elevated rates of structural variant calls"
author: "CIG @ MGI"
date: "`r Sys.Date()`"
...
Real.Data<-read.delim("SV_Intersection_Real_With_24kbBins.bed", header = FALSE)
colnames(Real.Data)<-c("Chr","Start","Stop","Count")
```

```{r read in permuted data, include=FALSE}
Shuffled.Data<-read.delim(gzfile("Combined_SV_Intersection_Shuffles_With_24kbBins.bed.gz"), header = FALSE)
...
ggsave(filename = "NullDistPlot_24kb.png",plot = NullDistPlot,units = "in", width = 12, height = 12)
...
ggsave(filename = "RealDataPlot_24kb.png",plot = RealDataPlot,units = "in", width = 12, height = 12)

RealDataPlot
```

```{r Save table of enrichment, include=FALSE}
write.table(x = Real.Data, file = "24kb_Regions_Enriched_in_SV_AllData.txt", quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE)
write.table(x = subset(Real.Data, Significant==TRUE), file = "24kb_Regions_Enriched_in_SV_SigOnly.txt", quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE)
```
...
ggsave(filename = "24kb_ManhattenLikePlots_Regions_Enriched_in_SV.png", plot = ManhattenPanel, units = "in", width = 12, height = 12)
```
