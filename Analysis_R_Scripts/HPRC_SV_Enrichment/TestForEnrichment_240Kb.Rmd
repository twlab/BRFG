---
title: "Testing 240Kb genomic regions for elevated rates of structural variant calls"
author: "CIG @ MGI"
date: "`r Sys.Date()`"
...
knitr::opts_knit$set(root.dir = "/Users/juanmacias/Library/CloudStorage/Box-Box/LawsonLab/JuanMacias/Postdoc/HPRC/SV_Enrichment")
rm(list = ls())
set.seed(0)
...
Real.Data<-read.delim("SV_Intersection_Real_With_240kbBins.bed", header = FALSE)
colnames(Real.Data)<-c("Chr","Start","Stop","Count")
```

```{r read in permuted data, include=FALSE}
Shuffled.Data<-read.delim(gzfile("Combined_SV_Intersection_Shuffles_With_240kbBins.bed.gz"), header = FALSE)
...
SampledNull<-Shuffled.Data[sample(1:nrow(Shuffled.Data), nrow(Real.Data)*100),]
...
ggsave(filename = "NullDistPlot_240kb.png",plot = NullDistPlot,units = "in", width = 12, height = 12)
...
ggsave(filename = "RealDataPlot_240kb.png",plot = RealDataPlot,units = "in", width = 12, height = 12)

RealDataPlot
```

```{r Save table of enrichment, include=FALSE}
write.table(x = Real.Data, file = "240Kb_Regions_Enriched_in_SV_AllData.txt", quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE)
write.table(x = subset(Real.Data, Significant==TRUE), file = "240Kb_Regions_Enriched_in_SV_SigOnly.txt", quote = FALSE, sep = "\t", col.names = TRUE, row.names = FALSE)
```
...
ggsave(filename = "240Kb_ManhattenLikePlots_Regions_Enriched_in_SV.png", plot = ManhattenPanel, units = "in", width = 12, height = 12)
```
